...
async function run() {
  try {
    ...
    console.log(vavooChannels.filter(c => c.name.toLowerCase().includes("foot")).map(c => c.name)); console.log(vavooChannels.filter(c => c.name.toLowerCase().includes("orange")).map(c => c.name)); return;
