...
export default function PlexPage({
  ...
  const [channels] = useState<Channel[]>(PLEX_CHANNELS);
  const [selectedChannel, setSelectedChannel] = useState<Channel | null>(
    PLEX_CHANNELS[0] || null,
  );
  ...
  const getPlayableUrl = (url: string) => {
    if (!url) return "";
    return `/api/proxy/stream?url=${encodeURIComponent(url)}&referer=${encodeURIComponent("https://plex.tv/")}`;
  };
  ...
  return (
    <div className="min-h-screen bg-[#070707] text-white flex flex-col relative overflow-hidden font-sans">
      ...
      <div className="absolute top-[-20%] left-[-10%] w-[900px] h-[900px] bg-orange-600/10 rounded-full blur-[180px] pointer-events-none animate-pulse duration-[8000ms]" />
      <div className="absolute bottom-[-15%] right-[-10%] w-[800px] h-[800px] bg-yellow-600/5 rounded-full blur-[160px] pointer-events-none" />
        ...
        {loading && (
          ...
          >
            <div className="relative">
              <div className="w-24 h-24 rounded-full border-4 border-t-orange-500 border-r-yellow-500 border-b-orange-300 border-l-transparent animate-spin" />
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="font-black text-xl italic tracking-tighter text-white">Px</span>
              </div>
            </div>
            <div className="text-center space-y-2">
              <h2 className="text-2xl font-black uppercase tracking-widest italic">
                PLEX <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-yellow-500">TV</span>
              </h2>
              <p className="text-white/40 text-xs font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-2">
                <Loader2 size={12} className="animate-spin" /> Connexion aux flux...
              </p>
            </div>
          </motion.div>
        )}
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto">
          ...
          <div className="flex items-center gap-3">
            <h1 className="text-xl md:text-2xl font-black tracking-[-0.05em] italic uppercase leading-none">
              PLEX <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-yellow-500 drop-shadow-md">TV</span>
            </h1>
            <div className="bg-white/5 border border-white/10 px-2 py-0.5 rounded text-[9px] font-black tracking-widest text-orange-400 uppercase">
              Free Live TV
            </div>
          </div>
        </div>
                ...
                return (
                  <div 
                    ...
                    className={cn(
                      ...
                      isSelected 
                        ? "border-orange-500/50 bg-orange-950/10 shadow-[0_8px_30px_rgb(249,115,22,0.15)]" 
                ...
                {isVideoLoading && (
                  <div className="absolute inset-0 bg-black/70 backdrop-blur-[2px] flex flex-col items-center justify-center gap-3 z-10 pointer-events-none">
                    <div className="relative flex items-center justify-center">
                      ...
                      <span className="absolute text-[10px] font-black italic text-orange-400">Px</span>
                    </div>
              ...
              <div className="p-6 md:p-8 flex-1 flex flex-col justify-start">
                <div className="flex items-start gap-5">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="bg-orange-500/20 text-orange-400 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest mb-2 inline-block">
                      {selectedChannel.category || 'Plex TV'}
                    </span>
                ...
                <div className="mt-8">
                  <button
                    ...
                    className="w-full py-4 bg-orange-600 hover:bg-orange-500 text-white font-black rounded-2xl transition-all active:scale-95 shadow-lg shadow-orange-600/20 flex items-center justify-center gap-3 uppercase tracking-widest text-xs"
                  >
