...
function readDb() {
  try {
    ...
    data.movies = data.movies.filter(
      (m: any) => m && m.id && !m.id.startsWith("seed-"),
    );
    ...
    if (data && data.channels) {
      ...
      let channels = data.channels.filter((c: any) => {
        ...
        return (
          !urlStr.includes("test-streams.mux.dev") ||
          nameStr.toLowerCase().includes("france 2")
        );
      });
...
async function syncServiceChannels(
  name: string,
  m3uUrl: string,
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith("#EXTINF:")) {
        const idMatch =
          line.match(/tvg-id="([^"]*)"/) || line.match(/id="([^"]*)"/);
        const logoMatch =
    ...
    fs.writeFileSync(
      path.join(process.cwd(), "src", `${fileName}Channels.ts`),
      tsContent,
      "utf-8",
    );
...
async function syncPlutoTV(io: Server) {
  ...
  try {
    ...
    plutoChannels
      .slice(0, 5)
      .forEach((c) => console.log(`Channel: ${c.name}, URL: ${c.url}`));
    const tsContent = `import { Channel } from './types';\n\nexport const PLUTO_CHANNELS: Channel[] = ${JSON.stringify(plutoChannels, null, 2)};\n`;
    fs.writeFileSync(
      path.join(process.cwd(), "src", "plutoChannels.ts"),
      tsContent,
      "utf-8",
    );
    ...
    for (const plutoChan of plutoChannels) {
      const categoryName = plutoChan.category
        ? `Pluto TV - ${plutoChan.category}`
        : "Pluto TV";
...
async function fetchAndIndexIptvOrg(): Promise<{
...
async function startServer() {
  ...
  app.get("/api/sports/epg/google", async (req, res) => {
      ...
      epgData: [
        {
          id: "fallback-epg-1",
          title:
            "Coupe du Monde de la FIFA 2026 - Groupe G : Match 50 (Houston)",
          sport: "football",
        ...
        },
        {
          id: "fallback-epg-2",
          title:
            "Coupe du Monde de la FIFA 2026 - Groupe H : Match 52 (Monterrey)",
          sport: "football",
          competition: "Coupe du Monde",
          date: "Ce soir",
          time: "21:00",
        ...
        {
          id: "fallback-epg-4",
          title: "Tour de France 2026 - Émission spéciale d'avant-course",
          sport: "cycling",
        ...
        {
          id: "fallback-epg-5",
          title: "Grand Prix d'Autriche F1 - Émission de présentation",
          sport: "f1",
    ...
    if (
      sportsEpgCache &&
      now - sportsEpgCache.timestamp < SPORTS_CACHE_TTL_MS
    ) {
      console.log(
        `[EPG Cache] Returning cached sports EPG data (Age: ${Math.round((now - sportsEpgCache.timestamp) / 1000)}s)`,
      );
      return res.json({
        ...sportsEpgCache.data,
        source: "google",
...
You MUST retrieve actual real-world matches using Google Search grounding. Do not invent fake matchups. Return valid JSON only.`;
    ...
    res.json({
      ...
      healthScore:
        db.channels.length > 0
          ? Math.round(((online + slow * 0.5) / db.channels.length) * 100)
          : 0,
    });
  });
  ...
  function findCorrectEpgProgs(channel: any): any[] {
    ...
    matchedKey = Object.keys(indexedProgrammes).find((k) => {
      const strippedK = k
        .replace(/\d+$/, "")
        .replace(/(fr|be|ch|ca|org|hd|fhd|sd)$/g, "");
      return areProgsMatching(strippedK, normName);
    });
    if (matchedKey) return indexedProgrammes[matchedKey];
  ...
  function generateFallbackEpgForChannel(
    channelName: string,
    category: string,
    now: Date,
  ) {
    ...
    if (normName.includes("disney junior")) {
      ...
      normName.includes("bein") ||
      normName.includes("rmc") ||
      normName.includes("canal")
    ) {
      titles = [
      ...
      normName.includes("cine+ frisson") ||
      normName.includes("cineplus frisson")
    ) {
      titles = [
      ...
      category.includes("Documentaires") ||
      normName.includes("discovery") ||
      normName.includes("science") ||
      normName.includes("histoire")
    ) {
      titles = [
      ...
      cats = [
        "Documentaire",
        "Histoire",
        "Sciences et Révélations",
        "Nature & Découvertes",
      ];
      progDuration = 45;
    } else if (
      category.includes("Musique") ||
      normName.includes("music") ||
      normName.includes("m6 music") ||
      normName.includes("nrj")
    ) {
      titles = [
  ...
  async function getLiveEpgData() {
    ...
    for (const channel of db.channels) {
      ...
      if (!ptvMatches) {
        ...
        const bestMatch =
          entries.find(([key]) => key === norm) ||
          entries.find(
            ([key]) =>
              key.startsWith(norm + "series") || key.startsWith(norm + "films"),
          ) ||
  ...
  async function scrapeProgrammeTv() {
    ...
    (async () => {
      try {
        ...
        // --- NEW: Scraping programme-television.org ---
        try {
          console.log(`Background scraping programme-television.org...`);
          const ptOrgRes = await axios.get(
          ...
          channelsBlocks.shift(); // remove first part
          ...
          for (let i = 0; i < channelsBlocks.length; i++) {
            ...
            if (title && !programs[normName]?.image) {
              // prefer image from programme-television.org if missing
              programs[normName] = {
                ...(programs[normName] || {}),
    ...
    try {
      ...
      for (const parsed of parsedChannels) {
        ...
        const duplicate = db.channels.find(
          (c: any) => c.name.toLowerCase() === parsed.name.toLowerCase(),
        );
  ...
  app.post("/api/playlists/parse", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: `Impossible d'analyser la playlist: ${err.message}` });
    }
  });
  ...
  app.post("/api/channels/repair", async (req, res) => {
    ...
    const offlineChannels = db.channels.filter(
      (c: any) => c.status === "offline" || !c.status,
    );
  ...
  app.post("/api/channels/merge", (req, res) => {
    ...
    primary.backupUrls = [
      ...new Set([...primary.backupUrls, ...secondaryUrls]),
    ];
  ...
  app.post("/api/channels/auto-merge-duplicates", (req, res) => {
    ...
    for (const c of db.channels) {
      ...
      if (!norm) {
        ...
        const fallbackKey = (c.name || "unknown")
          .toLowerCase()
          .replace(/[^a-z0-9]/g, "");
    ...
    for (const key in groups) {
      ...
      for (const candidate of group) {
        ...
        const candidateHasLogo =
          candidate.logo && !candidate.logo.includes("unsplash.com");
        const primaryHasLogo =
          primary.logo && !primary.logo.includes("unsplash.com");
        ...
        if (
          (candidateIsHD && !primaryIsHD) ||
          (candidateHasLogo && !primaryHasLogo)
        ) {
          primary = candidate;
        }
      }
  ...
  app.post("/api/epg/sources/toggle", (req, res) => {
    ...
    db.epgSources[req.body.index].isActive =
      !db.epgSources[req.body.index].isActive;
    writeDb(db);
  ...
  app.get("/api/proxy/stream", async (req, res) => {
    ...
    try {
      ...
      const isM6Url =
        url &&
        (url.includes("shls-m6-france") ||
          url.includes("origin2-6play") ||
          url.includes("shls-m6-int"));
      if (isM6Url) {
        console.log(
  ...
  app.get("/api/proxy/fstv", async (req, res) => {
    ...
    try {
      ...
      if (!path) {
        return res
          .status(404)
          .send("Stream not found or FSTV_SRC matching failed");
      }
      ...
      if (path.includes("ch=")) {
        ...
        if (nameMatch && nameMatch[1]) {
          ...
          if (sourcesRes.data && sourcesRes.data.length > 0) {
            const bestSource =
              sourcesRes.data.find((s: any) => s.q === "FHD" || s.q === "HD") ||
              sourcesRes.data[0];
  ...
  app.get("/api/proxy/fstv/seg", async (req, res) => {
    ...
    try {
      const response = await axios.get(
        `https://fstv.rest/live.php?seg=${encodeURIComponent(segUrl as string)}`,
        {
          responseType: "stream",
          headers: { Referer: "https://fstv.rest/" },
        },
  ...
  app.post("/api/channels/scrape-fstv", async (req, res) => {
    ...
    try {
      ...
      if (!newsidMatch)
        return res
          .status(400)
          .json({ error: "Invalid FSTV URL (missing newsid)" });
      ...
      let name = titleMatch
        ? titleMatch[1].split("regarder")[0].split("-")[0].trim()
        : `FSTV Channel ${newsid}`;
  ...
  app.get("/api/proxy/tvmio", async (req, res) => {
    ...
    try {
      const baseUrl =
        "https://tvmio.ooguy.com/eyJjb3VudHJpZXMiOlsiRlIiXSwmY2F0ZWdvcmllcyI6eyJGUiI6WyJHZW5lcmFsIPCfk7oiLCJTcG9ydHMg4pq9IiwiRG9jdW1lbnRhaXJlcyDwn4yNIiwiRmlsbXMg8J+OrCIsIkluZm9ybWF0aW9ucyDwn5OwIiwiTXVzaWMg8J+OtSIsIkVuZmFudHMg8J+RtiJdfSwiZW5hYmxlU2VhcmNoIjp0cnVlfQ";
      ...
      const bestStream =
        streams.find((s: any) => s.title.includes("FHD")) || streams[0];
  ...
  app.get("/api/proxy/vavoo", async (req, res) => {
    ...
    try {
      ...
      if (streamUrl) {
        ...
        return res
          .status(404)
          .send("VAVOO resolve failed to extract stream URL.");
      }
    } catch (err: any) {
      console.error("[VAVOO PROXY ERROR]", err.message);
      res
        .status(500)
        .send("VAVOO internal proxy error. Please try again later.");
    }
  });
  ...
  app.post("/api/channels/import-tvmio", async (req, res) => {
    try {
      ...
      if (metas.length === 0) {
        return res
          .status(404)
          .json({ error: "No channels found in TVMio catalog" });
      }
  ...
  setInterval(
    async () => {
      ...
      // Regular repair
      axios
        .post(`http://localhost:${PORT}/api/channels/repair`)
        .catch(() => {});
